...
def analyze_dg_deep_dive(model, test_loaders, device, save_dir):
    ...
    for i in range(model.num_experts):
        ...
        for j in range(i + 1, model.num_experts):
            ...
            for s in jaccard_sims.flatten().numpy():
...
def phase2_contrastive_tuning(model, train_loaders, device, args, log_dir):
    ...
    to enforce pattern separation. The feature extractor is frozen.
    ...
    logger.info(f"  🥶 Feature extractor, CA3, and output layers frozen.")
    ...
    for epoch in range(args.contrastive_epochs):
        ...
        for inputs, labels, task_ids in progress_bar:
            ...
            # Get features from the frozen extractor
            features = model.feature_extractor(inputs).view(inputs.size(0), -1)
            ...
            if expert_id > 0:
                ...
                if active_protos is not None and active_E > 0:
                    ...
                    if has_enough_counts:
                        ...
                        features = model.feature_extractor(sample_inputs).view(sample_inputs.size(0), -1)
...
def main():
        ...
        try:
            ...
            for expert_id in range(model.num_experts):
                if expert_id in model.dg_prototypes and model.dg_prototypes[expert_id] is not None:
                    ...
                    proto_array.append(np.zeros(dg_dim))
    ...
    # === FINAL PROTOTYPE UPDATE AFTER PHASE 2 ===
    # Update prototypes one final time after joint tuning
    model.update_dg_prototypes_from_ema()
    # model.compute_dg_prototypes(train_loaders, device)

    logging.info("✅ Final prototype update after Phase 2 complete")
    
    # Log final prototype statistics
    ...
    logging.info("🔬" * 60)
    phase2_results = analyze_dg_pattern_separation(model, test_loaders, device, log_dir)
    
    # Save Phase 2 prototype plots
    if hasattr(model, 'dg_prototypes') and model.dg_prototypes is not None:
        try:
            # Convert prototypes to numpy for plotting
            proto_array = []
            ...
            proto_array = np.array(proto_array)
            plot_prototype_stats(proto_array, phase='phase2', save_dir=log_dir, logger=logging)
            logging.info("✅ Phase 2 prototype plots saved")
        except Exception as e:
            logging.warning(f"⚠️ Failed to save Phase 2 prototype plots: {e}")

    # === ANALYSIS ===
    final_results = evaluate_dg_gated_model_standardized(model, test_loaders, task_classes, device)
    dg_analysis = analyze_dg_gated_model(model, test_loaders, device, log_dir)
    deep_dive_analysis = analyze_dg_deep_dive(model, test_loaders, device, log_dir)
    ...
    if 'silhouette_score' in phase1_results and 'silhouette_score' in phase2_results:
        sil_improvement = phase2_results['silhouette_score'] - phase1_results['silhouette_score']
        db_improvement = phase1_results['davies_bouldin_index'] - phase2_results['davies_bouldin_index']  # Lower is better
        ratio_improvement = phase2_results['separation_ratio'] - phase1_results['separation_ratio']
        
        logging.info(f"📈 Silhouette Score: {phase1_results['silhouette_score']:.4f} → {phase2_results['silhouette_score']:.4f} (Δ: {sil_improvement:+.4f})")
        logging.info(f"📈 Davies-Bouldin Index: {phase1_results['davies_bouldin_index']:.4f} → {phase2_results['davies_bouldin_index']:.4f} (Δ: {db_improvement:+.4f})")
        logging.info(f"📈 Separation Ratio: {phase1_results['separation_ratio']:.4f} → {phase2_results['separation_ratio']:.4f} (Δ: {ratio_improvement:+.4f})")
        
        if sil_improvement > 0 and db_improvement > 0 and ratio_improvement > 0:
            logging.info("🎉 EXCELLENT: Phase 2 improved all separation metrics!")
        elif sil_improvement > 0 or db_improvement > 0 or ratio_improvement > 0:
            logging.info("✅ GOOD: Phase 2 improved some separation metrics")
        else:
            logging.info("⚠️ WARNING: Phase 2 did not improve separation metrics")
        ...
        try:
            ...
            for expert_id in range(model.num_experts):
                if expert_id in model.dg_prototypes and model.dg_prototypes[expert_id] is not None:
                    ...
                    dg_dim = model.hippocampal_experts[0].dg.hidden_dim
                    proto_array_phase2.append(np.zeros(dg_dim))
...
def parse_arguments():
    ...
    parser.add_argument('--contrastive_epochs', type=int, default=15)
    ...
    parser.add_argument('--use_small_features', action='store_true', help='Use small feature extractor (128 channels instead of 256) for 48% FLOP savings')
